...
let s = (FStar_Seq_Base.seq_of_list l)
in s))
...
let of_list = (fun ( l  :  'a Prims.list ) -> (FStar_Seq_Base.seq_of_list l))
...
let sortWith = (fun ( f  :  'a  ->  'a  ->  Prims.int ) ( s  :  'a FStar_Seq_Base.seq ) -> (FStar_Seq_Base.seq_of_list (FStar_List_Tot_Base.sortWith f (FStar_Seq_Base.seq_to_list s))))
...
in (match (uu___1) with
| (hd, tl) -> begin
(FStar_Seq_Base.cons (f hd) (map_seq f tl))
end))
end))
