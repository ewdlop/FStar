...
let seq_to_list = (fun ( s  :  'uuuuu seq ) -> (match (s) with
| MkSeq (l) -> begin
l
end))


let seq_of_list = (fun ( l  :  'uuuuu Prims.list ) -> MkSeq (l))


let index = (fun ( s  :  'uuuuu seq ) ( i  :  Prims.nat ) -> (FStar_List_Tot_Base.index (__proj__MkSeq__item__l s) i))


let _cons = (fun ( x  :  'a ) ( s  :  'a seq ) -> MkSeq ((x)::(__proj__MkSeq__item__l s)))
...
| uu___ -> begin
(_cons v (create (len - (Prims.parse_int "1")) v))
end))
...
| uu___ -> begin
(_cons (contents k) (init_aux' len (k + (Prims.parse_int "1")) contents))
end))
...
| uu___ -> begin
(_cons (hd s) (upd' (tl s) (n - (Prims.parse_int "1")) v))
end))
...
let cons = (fun ( x  :  'a ) ( s  :  'a seq ) -> (append (create (Prims.parse_int "1") x) s))
...
| uu___1 -> begin
(_cons (hd s) (slice' (tl s) i (j - (Prims.parse_int "1"))))
end)
end))
