...
('a, 'rel, obj, obj) FStar_Monotonic_Heap.contains
...
let __proj__Ref__item___2 : some_ref  ->  (obj, obj) mreference = (fun ( projectee  :  some_ref ) -> (match (projectee) with
| Ref (a, rel, _2) -> begin
_2
end))
...
let norm_steps : FStar_NormSteps.norm_step Prims.list = (FStar_NormSteps.iota)::(FStar_NormSteps.zeta)::(FStar_NormSteps.delta)::((FStar_NormSteps.delta_only (("FStar.Monotonic.HyperStack.regions_of_some_refs")::("FStar.Monotonic.HyperStack.refs_in_region")::("FStar.Monotonic.HyperStack.modifies_some_refs")::[])))::(FStar_NormSteps.primops)::[]
